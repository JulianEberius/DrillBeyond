 ...
 */
static DrillBeyond *
_copyDrillBeyond(const DrillBeyond *from)
{
	DrillBeyond   *newnode = makeNode(DrillBeyond);
	/*
	 * copy node superclass fields
	 */
	CopyJoinFields((const Join *) from, (Join *) newnode);
	COPY_SCALAR_FIELD(drb_strategy);
	COPY_SCALAR_FIELD(drb_expansion); // just copying pointer, dangerous!
	COPY_SCALAR_FIELD(drb_expandNode);

	return newnode;
}

static DrillBeyondExpand *
_copyDrillBeyondExpand(const DrillBeyondExpand *from)
{
	DrillBeyondExpand   *newnode = makeNode(DrillBeyondExpand);
	...
	CopyPlanFields((const Plan *) from, (Plan*)newnode);
	COPY_SCALAR_FIELD(drb_strategy);
	COPY_SCALAR_FIELD(drb_expansions); // just copy list pointer! dangerous!
	COPY_SCALAR_FIELD(drb_all_expansions);
	COPY_SCALAR_FIELD(drb_expandFrom);
	COPY_SCALAR_FIELD(drb_expandTo);
	COPY_SCALAR_FIELD(drb_expansion);
	return newnode;
}

/*
 * _copyDrillBeyondDummy
 */
static DrillBeyondDummy *
_copyDrillBeyondDummy(const DrillBeyondDummy *from)
{
	DrillBeyondDummy    *newnode = makeNode(DrillBeyondDummy);
...
copyObject(const void *from)
{
	...
	switch (nodeTag(from))
	{
		...
		case T_HashJoin:
			...
			break;
		case T_DrillBeyond:
			retval = _copyDrillBeyond(from);
			break;
		case T_DrillBeyondExpand:
			retval = _copyDrillBeyondExpand(from);
			break;
		case T_DrillBeyondDummy:
			retval = _copyDrillBeyondDummy(from);
			break;
		case T_Material:
