			 ...
			 ParamListInfo params, DestReceiver *dest)
{
	...
	// if (is_drillbeyond()) {
	if (false) {
		do_text_output_drillbeyond_explain(tstate, es.str->data,
			drillbeyond_explain_string());
	} else {
		if (es.format == EXPLAIN_FORMAT_TEXT)
...
ExplainResultDesc(ExplainStmt *stmt)
{
	...
	if (is_drillbeyond()) {
		/* Need a tuple descriptor representing a two TEXT or XML column */
		tupdesc = CreateTemplateTupleDesc(2, false);
		TupleDescInitEntry(tupdesc, (AttrNumber) 1, "QUERY PLAN",
						   result_type, -1, 0);
		TupleDescInitEntry(tupdesc, (AttrNumber) 2, "DRILLBEYOND INFOS",
						   TEXTOID, -1, 0);
	} else {
			...
			ExplainState *es)
{
	...
	switch (nodeTag(plan))
	{
		...
		case T_HashJoin:
			...
			break;
		case T_DrillBeyond:
			pname = "DrillBeyond";		/* "Join" gets added by jointype switch */
			sname = "DrillBeyond Join";
			break;
		case T_DrillBeyondExpand:
			pname = "DrillBeyondExpand";		/* "Join" gets added by jointype switch */
			sname = "DrillBeyond Expand";
			break;
		case T_SeqScan:
	...
	switch (nodeTag(plan))
	{
		...
		case T_ModifyTable:
			...
			break;
		case T_DrillBeyond:
			appendStringInfo(es->str, " ω (%s)", ((DrillBeyond *)plan)->drb_expansion->keyword);
			break;
		case T_DrillBeyondExpand:
			{
				DrillBeyondExpand *expand = (DrillBeyondExpand *) plan;
				appendStringInfo(es->str, " %s", drb_strategyname(expand->drb_strategy));
				if (expand->drb_expansion != NULL)
					appendStringInfo(es->str, " (%s)", expand->drb_expansion->keyword);
			}
			break;
		case T_NestLoop:
		case T_MergeJoin:
	...
	switch (nodeTag(plan))
	{
		...
		case T_DrillBeyond:
			// show_upper_qual(((NestLoop *) plan)->join.joinqual,
			// 				"Join Filter", planstate, ancestors, es);
			if (((DrillBeyond *) plan)->join.joinqual)
				show_instrumentation_count("Rows Removed by Join Filter", 1,
										   planstate, es);
			show_upper_qual(plan->qual, "Filter", planstate, ancestors, es);
			if (plan->qual)
					  ...
					  List *ancestors, ExplainState *es)
{
	...
	for (keyno = 0; keyno < nkeys; keyno++)
	{
		...
		if (!target)
			continue; // TODO: DrillBeyond simply adds cols to sort that have no TLE
