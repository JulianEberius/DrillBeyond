...
set_plan_refs(PlannerInfo *root, Plan *plan, int rtoffset)
{
	...
	switch (nodeTag(plan))
	{
		...
		case T_DrillBeyondDummy:{
			DrillBeyondDummy *dbplan = (DrillBeyondDummy *) plan;
			dbplan->plan.targetlist =
				fix_scan_list(root, dbplan->plan.targetlist, rtoffset);
			dbplan->plan.qual =
				fix_scan_list(root, dbplan->plan.qual, rtoffset);
			break;
		}
...
set_join_references(PlannerInfo *root, Join *join, int rtoffset)
{
	...
	else if (IsA(join, DrillBeyond))
	{
		DrillBeyond *drb = (DrillBeyond *) join;
		DrillBeyondExpansion *exp = drb->drb_expansion;
		// TODO: mmove drb_qual from expansion to DrillBeyond node
		exp->drb_qual = fix_join_expr(root,
										exp->drb_qual,
										outer_itlist,
										inner_itlist,
										(Index) 0,
										rtoffset);
		drb->drb_join_cols = fix_join_expr(root,
										drb->drb_join_cols,
										outer_itlist,
										inner_itlist,
										(Index) 0,
