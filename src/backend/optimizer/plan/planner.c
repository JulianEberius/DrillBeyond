...
standard_planner(Query *parse, int cursorOptions, ParamListInfo boundParams)
{
	...
	drillbeyond_planner_phase_zero(parse);
	...
	// drb_printTL(top_plan, parse->rtable);
	top_plan = set_plan_references(root, top_plan);
	// printf("--->\n");
	// drb_printTL(top_plan, root->glob->finalrtable);
				 ...
				 PlannerInfo **subroot)
{
	...
	root->drb_expansions = NIL;
	root->drb_all_expansions = NIL;
...
grouping_planner(PlannerInfo *root, double tuple_fraction)
{
	...
	else
	{
		...
		/*
		 * Drillbeyond Operator
		 */
	    drillbeyond_planner_phase_one(root, tlist); // RENAME
	...
	/*
	 * Drillbeyond-Expand Operator
	 */
	if (root->drb_expansions != NULL
		|| (root->query_level == 1 && drb_is_variable_plan(root, result_plan))) {
		result_plan = drillbeyond_planner_phase_three(root, result_plan);
	}
