...
static DrillBeyondDummy *create_drillbeyond_dummy_plan(PlannerInfo *root, Path *best_path);
...
static Plan *create_drillbeyond_plan(PlannerInfo *root, DrillBeyondPath *best_path,
					 Plan *outer_plan, Plan *inner_plan);
...
static HashJoin *make_hashjoin(List *tlist,
			  ...
			  Plan *lefttree, Plan *righttree,
			  JoinType jointype);
static DrillBeyond *make_drillbeyond(List *tlist,
			  List *joinclauses, List *otherclauses,  List *drb_clauses,
			  Plan *lefttree, Plan *righttree,
			  JoinType jointype, DrillBeyondExpansion *drb_expansion);
static DrillBeyondDummy *make_drillbeyond_dummy(List *tlist,
			 List *qpqual,
			 Index scanrelid);
	...
	switch (best_path->pathtype)
	{
		...
		case T_DrillBeyond:
			plan = create_join_plan(root,
									(JoinPath *) best_path);
			break;
		case T_DrillBeyondDummy:
			plan = (Plan *) create_drillbeyond_dummy_plan(root, best_path);
			break;
		case T_Append:
...
/*
 */
static DrillBeyondDummy *
create_drillbeyond_dummy_plan(PlannerInfo *root, Path *best_path)
{
	DrillBeyondDummy *plan;
	RelOptInfo *rel = best_path->parent;
	...
	scan_clauses = extract_actual_clauses(scan_clauses, false);
 	...
 	drb_qual_vars = pull_var_clause((Node *) scan_clauses,
										   PVC_RECURSE_AGGREGATES,
										   PVC_RECURSE_PLACEHOLDERS);
 	add_vars_to_targetlist(root, drb_qual_vars, best_path->parent->relids, false);

	tlist = build_relation_tlist(rel);
	plan = make_drillbeyond_dummy(tlist,
										   scan_clauses,
										   scan_relid);

	copy_path_costsize(&plan->plan, best_path);
	return plan;
}
...
create_join_plan(PlannerInfo *root, JoinPath *best_path)
{
	...
	switch (best_path->path.pathtype)
	{
		...
		case T_HashJoin:
			...
			break;
		case T_DrillBeyond:
			plan = (Plan *) create_drillbeyond_plan(root,
												 (DrillBeyondPath *) best_path,
												 outer_plan,
												 inner_plan);
			break;
		case T_NestLoop:
...
static Plan *
create_drillbeyond_plan(PlannerInfo *root,
				 DrillBeyondPath *best_path,
				 Plan *outer_plan,
				 Plan *inner_plan)
{
	DrillBeyond   *join_plan;
	Plan       *result;
	List	   *tlist = build_relation_tlist(best_path->jpath.path.parent);
	List	   *joinrestrictclauses = best_path->jpath.joinrestrictinfo;
	List	   *joinclauses;
	List	   *otherclauses = NIL;
	List       *drb_clauses = NIL;
	Bitmapset  *drbTargetCols = NULL;
	Bitmapset *outer_drb_cols = NULL;
	...
	/*
 	 * Pull up the quals from the inner node, as the inner node of a
 	 * drillbeyond node is never executed
	 */
 	drb_clauses = inner_plan->qual;

	join_plan = make_drillbeyond(tlist,
							  joinclauses,
							  otherclauses,
							  drb_clauses,
							  outer_plan,
							  ...
							  best_path->drb_expansion);
	result = (Plan *) join_plan;
	copy_path_costsize(&join_plan->join.plan, &best_path->jpath.path);


	bms_free(outer_drb_cols);
	bms_free(drbTargetCols);

	return result;
}


extern DrillBeyondExpand *make_drillbeyondexpand(PlannerInfo *root,
					Plan *subplan, DrillBeyondStrategy drb_strategy,
					Bitmapset *cols, Bitmapset **added_tls, bool doSort, List *drb_ops, DrillBeyondExpansion* dbe)
{
	// TODO: Simplify this function, remove old code
	DrillBeyondExpand   *drillbeyondExpand = makeNode(DrillBeyondExpand);
	Plan *plan;
	Var *id_col_expr;
	int i, new_tlist_pos;

	drillbeyondExpand->drb_expansions = root->drb_expansions;
	drillbeyondExpand->drb_expansion = dbe;
	drillbeyondExpand->drb_all_expansions = root->drb_all_expansions;
	drillbeyondExpand->drb_strategy = drb_strategy;

	plan = &drillbeyondExpand->plan;
	plan->qual = NIL;
	plan->lefttree = subplan;
	plan->righttree = NULL;
	plan->targetlist = (List*) copyObject(subplan->targetlist);

	switch (drb_strategy) {
		case DRB_TOP: {
			int numExpansions = list_length(root->drb_all_expansions);
			// also consider subplan expansions
			// List *all_drb_expansions = NIL;
			// foreach(c, root->glob->subroots) {
		        // PlannerInfo *subplanInfo = (PlannerInfo *) lfirst(c);
		        // all_drb_expansions = list_concat(all_drb_expansions, subplanInfo->drb_expansions);
		        // numExpansions += list_length(subplanInfo->drb_expansions);
		    // }
			drillbeyondExpand->drb_expandFrom = (int*) palloc0(sizeof(int) * numExpansions);
			drillbeyondExpand->drb_expandTo = (int*) palloc0(sizeof(int) * numExpansions);

			// create ids for open vars in TL
		    List *newTlist = NIL;
			new_tlist_pos = list_length(plan->targetlist);
			for (i=0; i<numExpansions; i++) {
				DrillBeyondExpansion *sdrb = (DrillBeyondExpansion*) list_nth(root->drb_all_expansions, i);
				char *new_name;
				new_name = (char*)palloc(strlen(sdrb->extended_relname)+strlen(sdrb->keyword)+5);
				sprintf(new_name, "%s_%s_id", sdrb->extended_relname, sdrb->keyword);
				id_col_expr = makeVar(sdrb->rti, 2, 20, 0, 0, 0);
				newTlist = lappend(newTlist, makeTargetEntry(
														   (Expr *) id_col_expr,
														   new_tlist_pos + 1,
														   new_name,
														   false));
				drillbeyondExpand->drb_expandTo[i] = new_tlist_pos;
				new_tlist_pos++;
			}

			drillbeyondExpand->drb_numExpansions = numExpansions;
		    plan->targetlist = list_concat(plan->targetlist, newTlist);
		    break;
		}
		case DRB_EXPAND2: {
			drillbeyondExpand->plan.qual = (List*) copyObject(dbe->drb_qual);
			int col, numExpansions, k;
            Bitmapset *tmpset;
            if (cols == NULL) {
                cols = get_drb_tlist_entries(plan->targetlist);
                tmpset = bms_copy(cols);
            } else {
                tmpset = bms_copy(cols);
            }

            numExpansions = bms_num_members(tmpset);
            drillbeyondExpand->drb_expandFrom = (int*)palloc0(sizeof(int) * numExpansions);
            drillbeyondExpand->drb_expandTo = (int*)palloc0(sizeof(int) * numExpansions);
            drillbeyondExpand->drb_numExpansions = numExpansions;
            ...
            while ((col = bms_first_member(tmpset)) >= 0) {
                drillbeyondExpand->drb_expandFrom[k] = col-1;
                drillbeyondExpand->drb_expandTo[k] = col-1;
                k++;
            }
		...
		default: {
            ereport(ERROR,
                    (errcode(ERRCODE_INTERNAL_ERROR),
                        errmsg("Should never be reached!")
                    ));
		}
	}

	cost_drillbeyond_expand(root, drillbeyondExpand);

	return drillbeyondExpand;
}
...
make_drillbeyond(List *tlist,
			  ...
			  Plan *righttree,
			  JoinType jointype,
			  DrillBeyondExpansion *drb_expansion)
{
	DrillBeyond   *node = makeNode(DrillBeyond);
	Plan	   *plan = &node->join.plan;
	...
	node->join.jointype = jointype;
	node->join.joinqual = joinclauses;
    node->drb_expansion = drb_expansion;
    node->drb_strategy = DRB_DEFAULT;
    node->drb_expandNode = NULL; // set in planner if Big Omega is used
    node->drb_join_cols = (List *)copyObject(drb_expansion->join_cols);

	return node;
}

static DrillBeyondDummy *
make_drillbeyond_dummy(List *tlist,
			 List *qpqual,
			 Index scanrelid)
{
	DrillBeyondDummy  *node = makeNode(DrillBeyondDummy);
	Plan	          *plan = &node->plan;
