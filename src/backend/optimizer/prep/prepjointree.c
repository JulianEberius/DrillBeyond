						...
						AppendRelInfo *containing_appendrel)
{
	...
	subroot->drb_expansions = NIL;
	subroot->drb_all_expansions = NIL;
	...
	/* DrillBeyond: extended_rti needs to be fixed too */
	foreach(lc, subquery->rtable) {
		ListCell *llc;
	    RangeTblEntry *srte = (RangeTblEntry *) lfirst(lc);
	    if (srte->rtekind == RTE_DRILLBEYOND) {
	    	DrillBeyondExpansion *ex = srte->drb_expansion;
	    	if (ex->was_planned) // do not modify the expansion in reoptimizations, it persists between them
	    		continue;
			ex->rti += rtoffset;
	    	ex->extended_rti += rtoffset;
	    	foreach(llc, ex->join_cols) {
	    	    Node *node = (Node *) lfirst(llc);
	    		OffsetVarNodes((Node *) node, rtoffset, 0);
	    	}
	    	foreach(llc, ex->drb_qual) {
	    	    Node *node = (Node *) lfirst(llc);
	    		OffsetVarNodes((Node *) node, rtoffset, 0);
	    	}
// 			printf("adjusting rt by %d to %d and %d\pn", rtoffset, ex->rti, ex->extended_rti);
	    }
	}
