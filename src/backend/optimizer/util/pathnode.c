...
/*
 * create_drillbeyond_dummy_path
 */
Path *
create_drillbeyond_dummy_path(PlannerInfo *root, RelOptInfo *rel)
{
	Path	   *pathnode = makeNode(Path);

	pathnode->pathtype = T_DrillBeyondDummy;
	pathnode->parent = rel;
	pathnode->param_info = NULL;	/* never parameterized at present */
	pathnode->pathkeys = NIL;	/* for now, assume unordered result */

	// cost_drillbeyondummy(pathnode, root, rel);
...
DrillBeyondPath* create_drillbeyond_path(PlannerInfo *root,
                     RelOptInfo *joinrel,
                     JoinType jointype,
					 SpecialJoinInfo *sjinfo,
					 SemiAntiJoinFactors *semifactors,
					 ...
					 List *restrict_clauses)
{
    DrillBeyondPath *pathnode = makeNode(DrillBeyondPath);

    pathnode->jpath.path.pathtype = T_DrillBeyond;
	pathnode->jpath.path.parent = joinrel;
	...
	pathnode->jpath.innerjoinpath = inner_path;
	pathnode->jpath.joinrestrictinfo = restrict_clauses;
	pathnode->jpath.path.rows = pathnode->jpath.path.parent->rows;

	pathnode->drb_expansion = sjinfo->drb_expansion;

	final_cost_drillbeyond(root, pathnode, sjinfo, semifactors);
