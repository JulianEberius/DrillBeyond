...
static void match_unsorted_outer(PlannerInfo *root, RelOptInfo *joinrel,
					 ...
					 Relids param_source_rels);
static void drillbeyond_paths(PlannerInfo *root,
					 RelOptInfo *joinrel,
					 RelOptInfo *outerrel,
					 RelOptInfo *innerrel,
					 List *restrictlist,
					 ...
					 List *restrictlist)
{
	...
	/*
	 *  0. Consider DrillBeyond Paths, which are automatically the coolest ones.
	 */
	if (outerrel->rtekind == RTE_DRILLBEYOND) {
		return;
	}
	if (innerrel->rtekind == RTE_DRILLBEYOND) {
		drillbeyond_paths(root, joinrel, outerrel, innerrel,
...
static void
drillbeyond_paths(PlannerInfo *root,
					 RelOptInfo *joinrel,
					 RelOptInfo *outerrel,
					 RelOptInfo *innerrel,
					 List *restrictlist,
					 ...
					 Relids param_source_rels)
{
	...
	foreach(l, outerrel->pathlist)
    {
        ...
        /* parametrized paths cannot be used with drillbeyond */
        if (PATH_REQ_OUTER(outerpath))
        	continue;
         ...
         * qpquals rather than as true mergeclauses):
         */
		// TODO drillbeyond pathkeyes
        merge_pathkeys = build_join_pathkeys(root, joinrel, jointype,
                                             outerpath->pathkeys);

        add_path(joinrel, (Path *)
                 create_drillbeyond_path(root,
									  joinrel,
									  jointype,
									  sjinfo,
			 						  semifactors,
