...
join_search_one_level(PlannerInfo *root, int level)
{
	...
	foreach(r, joinrels[level - 1])
	{
		...
		/*
		 * DrillBeyond relations can not be considered
		 */
		if (old_rel->rtekind == RTE_DRILLBEYOND)
			continue;
						  ...
						  ListCell *other_rels)
{
	...
	for_each_cell(l, other_rels)
	{
		...
		{
			tmp = make_join_rel(root, old_rel, other_rel);
		}
	}
}
							  ...
							  ListCell *other_rels)
{
	...
	for_each_cell(l, other_rels)
	{
		...
		if (!bms_overlap(other_rel->relids, old_rel->relids) &&
			!(other_rel->rtekind == RTE_DRILLBEYOND))
		{
			tmp = make_join_rel(root, old_rel, other_rel);
		}
	}
...
make_join_rel(PlannerInfo *root, RelOptInfo *rel1, RelOptInfo *rel2)
{
	...
	// Assert(rel1->rtekind != RTE_DRILLBEYOND);
	// check for dependency not necessary here anymore, right?
	...
	if (sjinfo == NULL)
	{
		...
		if (rel2->rtekind == RTE_DRILLBEYOND) {
			ListCell *c;
			sjinfo->is_drillbeyond = true;
			foreach (c, root->drb_expansions) {
        		DrillBeyondExpansion *expansion = lfirst(c);
        		if (expansion->rti == rel2->relid) {
            		sjinfo->drb_expansion = expansion;
            		break;
        		}
        	}
		} else {
			sjinfo->is_drillbeyond = false;
		}
	}
