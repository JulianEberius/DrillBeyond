...
static void set_function_pathlist(PlannerInfo *root, RelOptInfo *rel,
					  RangeTblEntry *rte);
static void set_drillbeyond_dummy_pathlist(PlannerInfo *root, RelOptInfo *rel,
					  RangeTblEntry *rte);
			 ...
			 Index rti, RangeTblEntry *rte)
{
	...
	else
	{
		switch (rel->rtekind)
		{
			...
			{
				RelOptInfo *baserel=root->simple_rel_array[rte->drb_expansion->extended_rti];
				rel->rows = baserel->rows * clauselist_selectivity(root,
				...
				rel->width = baserel->width + get_typavgwidth(1700, -1);

				set_drillbeyond_dummy_pathlist(root, rel, rte);
				break;
			}
			case RTE_CTE:
...
/*
 * set_drillbeyond_pathlist
 */
static void
set_drillbeyond_dummy_pathlist(PlannerInfo *root, RelOptInfo *rel, RangeTblEntry *rte)
{
	/* Generate appropriate path */
	add_path(rel, create_drillbeyond_dummy_path(root, rel));
