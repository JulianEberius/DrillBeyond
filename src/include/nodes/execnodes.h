...
typedef ScanState DrillBeyondDummyState;
...
/* ----------------
 *	 DrillBeyond information
 * ----------------
 */
typedef struct DrillBeyondState
{
	JoinState		js;				/* its first field is NodeTag */
	TupleTableSlot  *db_InnerTupleSlot;
	TupleTableSlot  *db_OuterTupleSlot;
	List            *db_qual;
	bool 			db_NeedNewOuter;
	bool 			db_fetchedResult;
	int				db_num_cands;
	int				db_tuples_requested;
	int             db_valueColIdx;
	int  			db_current_context;
	int             db_current_origin;
	struct DrillBeyondValues *db_current_values;
    int         num_join_cols;
    struct DrillBeyondExpandState *drb_expand_operator_state;
    Datum *keys;
    Tuplestorestate *tuplestorestate; // include tuplestore directly into drb
} DrillBeyondState;

typedef struct DrillBeyondExpandState
{
	PlanState		ps;				/* its first field is NodeTag */
	/* for compression */
	struct DrillBeyondVariantStore *stores;
	Tuplestorestate *tupstore;
	int  storedTuples;
	Datum *compressedVals;
	bool needNewContext;
	TupleTableSlot *intermediate_slot;
	TupleTableSlot *temp_slot;
	int current_origin;
	int total_permutations;
	List *drb_operator_states;
	DrillBeyondState *drb_operator_state;
	List *drb_quals;
	bool materialized;
	/* sorting */
	...
	int             current_val;
	int original_eflags;
	bool fragments_executed;
} DrillBeyondExpandState;
