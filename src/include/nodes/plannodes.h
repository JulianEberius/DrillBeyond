...
/* ----------------
 *		drill beyond dummy node
 * ----------------
 */
typedef Scan DrillBeyondDummy;
 ...
 * ----------------
 */
typedef enum DrillBeyondStrategy
{
	DRB_EXPAND,
	DRB_TOP,
	DRB_DEFAULT,
...
typedef struct DrillBeyond
{
	Join		join;
	DrillBeyondStrategy drb_strategy;
	struct DrillBeyondExpansion *drb_expansion;
	struct DrillBeyondExpand *drb_expandNode; // reference to Big Omega, if existing
	struct DrillBeyondExpand *drb_topNode; // reference to Big Omega, if existing
	List *drb_addedMatNodes; // materialization nodes that were added to the plan by DrillBeyond
	List *drb_join_cols; //vars that determine the expanded value (columns of the original table that "join" with web table)
} DrillBeyond;

typedef struct DrillBeyondExpand
{
	Plan				plan;
	DrillBeyondStrategy drb_strategy;
	List  	 		   *drb_expansions; /*list of DrillBeyondExpansion*/
	List  	 		   *drb_all_expansions; /*list of DrillBeyondExpansion*/
	int        		   *drb_expandFrom; /* offset into targetlist */
	int			       *drb_expandTo;
	int         		drb_numExpansions;
	struct DrillBeyondExpansion *drb_expansion;
	PlannedStmt *reoptimized_plan; /* set by lower level DrillBeyond nodes if reoptimization is necessary */
} DrillBeyondExpand;
